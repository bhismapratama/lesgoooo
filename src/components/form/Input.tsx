...
export default function Input({
  ...
  return (
    <div className='w-full space-y-1'>
        ...
        {type === 'file' ? (
          <label htmlFor={id} className='relative cursor-pointer'>
            <div className='w-full pl-9 pr-3 py-1.5 rounded-lg ring-1 ring-inset ring-typo-inline'>
              ...
              />
              {value?.[0] ? (
                <Typography variant='c12' className='relative'>
                  {value?.[0]?.['name']}
                </Typography>
              ) : (
                <Typography variant='c12' className='relative text-typo-icon'>
      ...
      {error && !hideError && (
        <Typography as='p' variant='c12' className='w-full text-critical-60'>
          {error.message}
        </Typography>
      )}
    </div>
